...
export interface AuthConfig<S = AnyObject> {
  /** 传递给验证器的额外参数 */
  extra?: any;
  /** 局部验证器 */
 ...
 * auth实例
 * */
export interface Auth<S = AnyObject, V = AnyObject> {
  /**
   * @param authKeys - 所属权限, 如果数组项为数组则表示逻辑 `or`
   * @return validMeta - 验证结果，如果验证通过则为null
   * */
  (authKeys: AuthKeys<V>): ValidMeta[] | null;
  /**
   * @param authKeys - 所属权限, 如果数组项为数组则表示逻辑 `or`
   * @param config - 配置
   * @return validMeta - 验证结果，如果验证通过则为null
 ...
 * auth创建配置
 * */
export interface CreateAuthConfig<S = any, V = any> {
  /** 用于控制内部状态的seed */
  seed: Seed<S>;
  /** 待注册的验证器 */
  validators?: V;
